...
// The core product surface (README Hero 2): interactive map with layer +
// category filters, click-to-report, and marker popups for both raw reports
// and clusters. Owns its own layer/filter/draft-report state since all of it
// is local interaction that doesn't need to live above this component.
export default function MapCanvas({ reports, clusters }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative isolate h-[520px] w-full overflow-hidden rounded-sm border border-line-dark/60">
        <MapContainer
          center={MOCK_CENTER}
          zoom={14}
        ...
        >
          ...
          {visibleClusters.map((cluster) => (
            ...
            >
              <Popup>
                <p className="text-xs font-semibold uppercase tracking-wide">
                  {cluster.category.replace("_", " ")} · {cluster.status}
                </p>
                <p className="mt-1 text-sm">{cluster.description}</p>
                <p className="mt-2 text-xs opacity-70">
                  Confidence {cluster.confidence}/100 · Verified by{" "}
        ...
        {draftLocation && (
          <div className="absolute inset-y-0 right-0 z-[1000] w-full max-w-sm overflow-y-auto border-l border-line-dark/60 bg-ink/95 p-5 text-ink-text backdrop-blur">
            {confirmedReport ? (
              <ReportConfirmation
              ...
              <>
                <p className="mb-4 font-mono text-xs uppercase tracking-[0.1em] opacity-60">
      ...
      <p className="font-mono text-xs uppercase tracking-[0.1em] opacity-50">
