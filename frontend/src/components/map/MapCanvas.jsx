...
export default function MapCanvas({ reports, clusters }) {
  const [activeLayers, setActiveLayers] = useState(["raw", "candidate", "verified"]);
  const [activeCategories, setActiveCategories] = useState(() =>
    Array.from(new Set(reports.map((r) => r.category).concat(clusters.map((c) => c.category)))),
  );
  ...
  const visibleClusters = useMemo(
    () =>
      clusters.filter(
        (c) => activeLayers.includes(c.status) && activeCategories.includes(c.category),
      ),
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <CategoryFilterChips activeCategories={activeCategories} onToggle={toggleCategory} />
      </div>
      ...
      <div className="relative isolate h-[520px] w-full overflow-hidden rounded-2xl">
        ...
        >
          ...
          {visibleReports.map((report) => (
            <Marker key={report.id} position={[report.lat, report.lng]} icon={reportIcon(report.category)}>
              <Popup>
                <p className="text-xs font-semibold capitalize">{report.category.replace("_", " ")}</p>
            ...
            >
              <Popup>
                ...
                <p className="mt-2 text-xs opacity-70">
                  Confidence {cluster.confidence}/100 &middot; Verified by {cluster.distinctReporters}{" "}
        ...
        {draftLocation && (
          <div className="absolute inset-y-0 right-0 z-[1000] w-full max-w-sm overflow-y-auto rounded-r-2xl bg-ink-card/95 p-5 text-ink-text backdrop-blur">
            {confirmedReport ? (
              ...
              <>
                <p className="mb-4 text-sm font-semibold">New report at this point</p>
                <NearbyReportsList
      ...
      <p className="text-sm text-ink-text/50">Click anywhere on the map to file a report at that location.</p>
    </div>
  );
}
