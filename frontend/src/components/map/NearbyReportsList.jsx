...
export default function NearbyReportsList({ lat, lng }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getNearbyReports({ lat, lng, radiusM: NEARBY_RADIUS_M }).then(({ data }) => {
      if (!cancelled) {
        setReports(data);
        setLoading(false);
      }
  ...
  if (loading) {
    return <p className="mb-4 text-sm opacity-50">Checking for nearby reports.</p>;
  }

  if (reports.length === 0) {
    return (
      <p className="mb-4 text-sm opacity-60">
        No reports within {NEARBY_RADIUS_M}m of this point yet. You&apos;d be the first.
      </p>
    );
  }
      ...
      <ul className="flex flex-col gap-2">
        {reports.slice(0, MAX_SHOWN).map((r) => (
          <li key={r.id} className="rounded-xl bg-current/5 p-3 text-sm">
            ...
            {r.description && <p className="mt-1 opacity-70">{r.description}</p>}
          </li>
        ))}
      </ul>
    </div>
