<!-- SETUP.md
     Purpose: step-by-step developer setup for VeriGrid.
     Copy this file into the repository root (e.g., /SETUP.md) so contributors can run the project locally.
-->
...
1. Ensure system build deps for `psycopg2` (Debian/Ubuntu example):
   ```bash
...
3. Install Python dependencies:
   ...
   ```
4. Configure environment: create `backend/.env` with `DATABASE_URL` (and any other required env vars).
5. Run uvicorn from the backend directory:
   ```bash
   cd backend
...
1. Install Node.js (Node 18+ or latest LTS; Next 16 may expect a modern Node version).
...
1. Create a feature branch locally (don't fork):
   ```bash
...
2. Commit and push to origin:
   ...
   ```
3. Open a pull request on GitHub from that branch.
