...
- **MirEye observation payload**: the moment a cluster verifies, VeriGrid builds and persists the full structured observation MirEye would need (category, location, confidence, distinct reporters, timestamps), inspectable via `GET /clusters/{id}/mireye-payload`, ready to send the instant MirEye exposes a write endpoint.
